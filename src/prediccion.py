...
def normalizar_categoria(valor: object, opciones: List[str]) -> str:
    """Normaliza entrada categórica y manda categorías no vistas a OTROS."""
...
def preparar_entrada(entrada: Dict[str, object], metadata: Dict[str, object]) -> pd.DataFrame:
    """Convierte el diccionario del formulario en un DataFrame válido para el pipeline."""
...
def clasificar_productividad(valor: float, metadata: Dict[str, object]) -> str:
    """Clasifica la productividad estimada usando percentiles del histórico."""
...
def predecir_productividad(entrada: Dict[str, object], modelo=None, metadata=None) -> Dict[str, object]:
    ...
    return {
        "productividad_m3_h": pred,
        "clasificacion": clasificar_productividad(pred, metadata),
        "entrada_modelo": X.iloc[0].to_dict(),
    }


def equipos_disponibles(metadata: Dict[str, object], filtro_marca: Optional[str] = None) -> List[str]:
    """Lista equipos. Si se pasa marca, filtra equipos cuya marca modal coincida."""
...
def ranking_equipos(
    ...
    """Compara equipos manteniendo constantes las condiciones del escenario."""
    if modelo is None or metadata is None:
        modelo, metadata = cargar_sistema()
    ...
    for equipo in equipos:
        fila = dict(escenario)
        fila["EQUIPO"] = equipo
        # Para no comparar equipos con una marca imposible, se usa la marca histórica/modal del equipo.
